...
def paso_2_busqueda(X_train, y_train, cv):
    ...
    # --- (2.1) Submuestreo estratificado para la  búsqueda de hiperparámetros  ---
    # Si N_MUESTRA_CV es None, no se submuestrea: la búsqueda usa todo el train.
    # Si N_MUESTRA_CV es un entero, se toma submuestra estratificada de ese tamaño. 
    # El ajuste final siempre usa todo el train.
    if N_MUESTRA_CV is not None and len(X_train) > N_MUESTRA_CV:
        from sklearn.model_selection import train_test_split as _split
        X_busq, _, y_busq, _ = _split(
            X_train, y_train,
            train_size=N_MUESTRA_CV, random_state=SEMILLA, stratify=y_train,
        )
        info(f"Submuestreo para la búsqueda: {len(X_busq):,} filas "
            f"(fraude: {y_busq.mean():.4f})")
    else:
        X_busq, y_busq = X_train, y_train
    ...
    # --- (2.3) Ajuste final con los mejores hiperparámetros ---
...
def _proba_via_cv(modelo, X, y, cv):
    ...
    if N_MUESTRA_CV is not None and len(X) > N_MUESTRA_CV:
        ...
        X_sub, y_sub = X, y
    # El estimador ya usa n_jobs=-1, asi que dejamos cross_val_predict en n_jobs=1; 
    # cada HistGradientBoosting ya usa varios hilos.
    probas = cross_val_predict(modelo, X_sub, y_sub, cv=cv,
                                method="predict_proba", n_jobs=1)[:, 1]
    return probas, np.asarray(y_sub)


def paso_3_umbral(modelo, X_train, y_train, cv, etiqueta):
    encabezado(f"PASO 3 - Umbral óptimo para {etiqueta} (maximiza F1 en CV)")
    info("Generando probabilidades fuera de muestra (cross_val_predict)...")
    probas, y_sub = _proba_via_cv(modelo, X_train, y_train, cv)
...
def main():
    ...
    umbral, metricas = paso_3_umbral(modelo, X_train, y_train, cv, "Gradient Boosting")
