...
#Iteración 2
#GRID_C           = [5.0, 10.0, 25.0, 50.0, 100.0]
#GRID_L1_RATIO    = [0.8, 0.85, 0.9, 0.95, 1.0]
GRID_C           = [50.0]
GRID_L1_RATIO    = [1.0]
SOLVER_EN        = "saga"  # único solver que admite Elastic Net en sklearn.
MAX_ITER_EN      = 5000
TOL_EN           = 1e-4
N_MUESTRA_CV     = 100000  # None = todo el train. Si no queremos, poner otro valor (por ej. 100000).
GRID_UMBRAL      = np.arange(0.01, 1.0, 0.01)
P_VALOR_MAX      = 0.05    # umbral de significación para backward elimination
...
def paso_1_carga():
    ...
    X_train = pd.read_csv(os.path.join(CARPETA_DATOS, "X_train.csv"), index_col=0)
    X_test  = pd.read_csv(os.path.join(CARPETA_DATOS, "X_test.csv"),  index_col=0)
    y_train = pd.read_csv(os.path.join(CARPETA_DATOS, "y_train.csv"), index_col=0).squeeze()
...
def paso_4_elastic_net(X_train_std, y_train, cv):
    ...
    info(f"Grid: C={GRID_C}, l1_ratio={GRID_L1_RATIO} -> "
        f"{len(GRID_C)*len(GRID_L1_RATIO)} combinaciones x {cv.n_splits} folds "
        f"= {len(GRID_C)*len(GRID_L1_RATIO)*cv.n_splits} ajustes en total")
    if N_MUESTRA_CV is None:
        info("AVISO: búsqueda sobre el train completo. Con ~462K filas y usando SAGA, "
            "esta etapa puede tardar entre 30 minutos y varias horas.")

    # --- (4.1) Submuestreo estratificado para la búsqueda de hiperparámetros ---
    if N_MUESTRA_CV is not None and len(X_train_std) > N_MUESTRA_CV:
        from sklearn.model_selection import train_test_split as _split
        X_busq, _, y_busq, _ = _split(
            X_train_std, y_train,
            train_size=N_MUESTRA_CV,
            random_state=SEMILLA,
            stratify=y_train,
        )
        info(f"Submuestreo para la búsqueda: {len(X_busq):,} filas "
            f"(fraude: {y_busq.mean():.4f})")
    else:
        X_busq, y_busq = X_train_std, y_train
        info(f"Búsqueda sobre el train completo: {len(X_busq):,} filas")
    ...
    # --- (4.3) Ajuste final con los mejores hiperparámetros ---
    info("Reentrenando con los mejores hiperparámetros sobre el train completo...")
...
# ------------------------------------------------------------------------------------------------
def _proba_via_cv(modelo, X, y, cv):
    """Probabilidades fuera de muestra mediante CV (sin sobreajuste).
    Si N_MUESTRA_CV es un entero y el dataset lo supera, submuestrea para acelerar.
    Si es None, se usa todo el dataset."""
    if N_MUESTRA_CV is not None and len(X) > N_MUESTRA_CV:
        from sklearn.model_selection import train_test_split as _split
        X_sub, _, y_sub, _ = _split(
            X, y, train_size=N_MUESTRA_CV, random_state=SEMILLA, stratify=y,
        )
    else:
        X_sub, y_sub = X, y
    probas = cross_val_predict(modelo, X_sub, y_sub, cv=cv,
                                method="predict_proba", n_jobs=-1)[:, 1]
    return probas, y_sub.values


def paso_5_umbral(modelo, X_train_std, y_train, cv, etiqueta, beta=BETA_FBETA):
    encabezado(f"PASO 5 - Umbral óptimo para {etiqueta} (maximiza F-{beta} en CV)")
    info("Generando probabilidades fuera de muestra (cross_val_predict)...")
    probas, y_sub = _proba_via_cv(modelo, X_train_std, y_train, cv)
    info(f"   ({len(probas):,} probabilidades calculadas)")

    precisiones, recalls, f1s, fbetas = [], [], [], []
    for u in GRID_UMBRAL:
        y_pred = (probas >= u).astype(int)
        precisiones.append(precision_score(y_sub, y_pred, zero_division=0))
        recalls.append   (recall_score   (y_sub, y_pred, zero_division=0))
        f1s.append       (f1_score       (y_sub, y_pred, zero_division=0))
        fbetas.append    (fbeta_score    (y_sub, y_pred, beta=beta, zero_division=0))
...
#  PASO 8 - GUARDADO
# ------------------------------------------------------------------------------------------------
def paso_8_guardar(modelo_sm_sklearn, modelo_sm, modelo_en, resultados, busqueda, scaler):
    ...
    joblib.dump(modelo_sm, os.path.join(CARPETA_RESULT, "modelo_sin_reg_statsmodels.joblib"))
    ...
    tabla = pd.DataFrame([{
        ...
        "Especificidad": r["Especificidad"], "F1": r["F1"], "F2": r["F2"],
...
def main():
    ...
    umbral_sm, metricas_sm = paso_5_umbral(modelo_sm_sklearn, X_train_std[vars_glm], y_train, cv,
                            "regresión logística sin regularización (backward)")
    umbral_en, metricas_en = paso_5_umbral(modelo_en, X_train_std, y_train, cv,
    ...
    # Guardado.
    paso_8_guardar(modelo_sm_sklearn, modelo_sm, modelo_en, [res_sm, res_en], busqueda, scaler)
